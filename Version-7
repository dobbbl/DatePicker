...
pick: function(date) {
  var startDate = new Date('2023-07-17');
  var endDate = new Date('2023-08-03');

  if (date >= startDate && date <= endDate) {
    alert('Please select a date outside the restricted range (17 July - 3 August).');
    return false;
  }
  ...
  var handlers = {
    click: function click(e) {
      ...
      // setting curr date 
      if (gCurrentDate.set) {
        gCurrentDate.set = false;
        gCurrentDate.Day = viewDay;
        gCurrentDate.year = viewYear;
        gCurrentDate.month = viewMonth;
      }
console.log(view);
      switch (view) { 
        case "years prev":
        ...
        case "day":
          ...
          // check slected date
          if (
            viewDay <= gCurrentDate.Day &&
            viewMonth <= gCurrentDate.month &&
            viewYear <= gCurrentDate.year
          ) {
            return;
          } else if (
            viewMonth < gCurrentDate.month &&
            ...
            return;
          } else if (viewYear < gCurrentDate.year) {
            return;
          }
          ...
          const datetemp = new Date(viewYear, viewMonth, viewDay);
          const dayIndex = datetemp.getDay();
          const dayName = dayOfWeektemp[dayIndex];
          if (dayName == "Monday") {
            return;
          }
